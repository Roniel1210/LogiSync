...
import { Search, Network, FileIcon, Filter } from 'lucide-react';
import { motion } from 'motion/react';
import { cn } from '../lib/utils';

const documents = [
  { id: 1, title: 'Contrato Marco Logistics 2024', type: 'CONTRATO', time: 'HACE 2H', size: '2.4 MB', match: 94 },
  { id: 2, title: 'Guía de Remisión GR-8821', type: 'TRANSPORTE', time: 'HACE 5H', size: '145 KB', match: 88 },
  ...
  { id: 5, title: 'Reporte Q3 Satisfacción', type: 'CLIENTE', time: 'HACE 2 DÍAS', size: '4.5 MB', match: 42 },
];
  ...
  return (
    ...
    >
      <div className="flex-shrink-0">
        ...
        <p className="text-sm text-slate-500 mt-1">Base de conocimiento semántica para análisis y extracción documental.</p>
      </div>
      ...
      <div className="flex-shrink-0 bg-white rounded-xl p-3 border border-slate-100/60 shadow-[0px_2px_8px_0px_rgba(0,0,0,0.04)] flex flex-col sm:flex-row items-center gap-3">
        <div className="flex-1 w-full relative">
          ...
          <input 
            type="text" 
            placeholder="Ej: Extraer términos de penalización en contratos de transporte del último mes..."
        ...
        <button className="bg-[#4f46e5] hover:bg-indigo-600 text-white px-8 py-3.5 rounded-lg text-sm font-bold tracking-wider uppercase transition-colors shadow-sm w-full sm:w-auto">
          Búsqueda
        </button>
      </div>
        ...
        <div className="col-span-1 lg:col-span-5 bg-white rounded-xl shadow-[0px_2px_8px_0px_rgba(0,0,0,0.04)] border border-slate-100/60 flex flex-col overflow-hidden">
           <div className="p-5 border-b border-slate-100 flex items-center gap-2">
             <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2">
               <Network className="w-4 h-4 text-indigo-500" /> Representación Vectorial
             </h2>
           </div>
