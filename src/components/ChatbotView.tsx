...
import { Send, Bot, User, Sparkles, BarChart2, TrendingUp, Package, RefreshCw, Server, WifiOff } from 'lucide-react';
...
const fallbackAnswer = 'n8n no respondio. Verifica el webhook `chatbot-query` o deja activo el fallback local mientras importas el workflow.';
...
export function ChatbotView() {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await postN8n<ChatbotResponse>('/webhook/chatbot-query', {
      ...
      setMessages([
        ...
        {
          role: 'assistant',
          content: response.answer?.trim() || fallbackAnswer,
        },
      ]);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo consultar n8n');
      setMessages([
        ...newMessages,
        {
          role: 'assistant',
    ...
    >
      <div className="flex-shrink-0 mb-6 flex items-start justify-between gap-4">
        ...
        <div className={cn(
          ...
          error ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'
        )}>
          {error ? <WifiOff className="w-4 h-4" /> : loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Server className="w-4 h-4" />}
          <span>{error ? 'Fallback local' : loading ? 'Consultando n8n...' : 'Chat conectado a n8n'}</span>
        </div>
      </div>
      {error && <p className="mb-4 text-xs text-amber-700">{error}</p>}
